...
int main(int, char**)
{
    ...
    int nk_ver[4]{0};
    nk_get_version(nk_ver);
    std::string title = "NK v." + std::to_string(nk_ver[0]) + "." + std::to_string(nk_ver[1]) + "." + std::to_string(nk_ver[2]) + "." + std::to_string(nk_ver[3]);

    // Create window
    float main_scale = ImGui_ImplGlfw_GetContentScaleForMonitor(glfwGetPrimaryMonitor()); // Valid on GLFW 3.3+ only
    wgpu_surface_width *= main_scale;
    wgpu_surface_height *= main_scale;
    GLFWwindow* window = glfwCreateWindow(wgpu_surface_width, wgpu_surface_height, title.c_str(), nullptr, nullptr);
    if (window == nullptr)
        return 1;
...
void nk_on_ws_message(unsigned char* data, int len)
{
    RequestManager::ProcessRequest(data, len);
}
